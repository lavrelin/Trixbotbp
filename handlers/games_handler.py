...
async def wordclear_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("📝 Использование: `/play3xiawordclear слово`")
        return
...
async def anstimeset_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("📝 Использование: `/play3xiaanstimeset 60` (в минутах)")
        return
...
async def roll_draw_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("📝 Использование: `/play3xiaroll 3` (количество победителей 1-5)")
        return
