...
PIAR_STEPS = [
    ('name', 'Имя', '🙅 Каталог Услуг – место, где каждый найдет нужного специалиста, а мастера получат своих постоянных клиентов.⚙️Чтобы попасть в каталог, заполни короткую анкету из 8 пунктов.✍️ После проверки и модерации твоя заявка будет опубликована, и ты получишь уведомление. 🙋🏼‍♂️Шаг первый из 8: Укажи своё имя, псевдоним или никнейм.'),
    ('profession', 'Профессия', '💬 Какие услуги вы предоставляте❔'),
    ('districts', 'Районы', '💬 В каких районах вы работаете❔ (до 3, через запятую):'), 
    ('phone', 'Телефон', '💬 Укажите номер телефона, если используете его в работе с клиентами, или отправьте «-» чтобы пропустить.'),
    ('instagram', 'Instagram', '💬 Введите ваш 💥Instagram. 🔗 Можно указать: ссылку, @username или просто username. ➖ Если не хотите указывать – отправьте «-».'),
    ('telegram', 'Telegram', '💬 Введите ваш 🌀Telegram. 🔗 Можно указать: ссылку, @username или просто username. ➖ Если не хотите указывать – отправьте «-».'),
    ('price', 'Цена', '"💬 Укажите цену за услуги, 📑 прайс-лист (если есть):"'),
    ('description', 'Описание', '"💬 Опишите ваши услуги, 📷 добавьте медиа, ✨ используйте эмодзи и абзацы — чем ярче описание, тем больше клиентов оно привлечёт:"')
]
