...
async def show_write_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "Публикуем в канал 💁‍♂️Будапешт: Объявления, новости, подслушанно, жалобы. В канал 🕵🏽‍♀️Куплю/ Отдам/ Продам - все заявки связаны с торгами, обменами, раздачами и тд. \n\n"
        
        "💥 *Заявка в 🙅🏻‍♀️Каталог Услуг*\n"
        "Собираем специалистов разных направлений для удобного поиска пользователями❕ (примеры: делаю маникюр, репетитор английского, переводчик, тд). \n\n"
        
        "⚡️ *Актуальное*\n"
        "Важные и срочные сообщения который публикуются в наш чат и закрепляются❕ (примеры: нужен стоматолог на сегодня кто может принять❔; Потерял 🐶 в 13 районе➕ фото; На сегодня нужен 🚐 по Будапешту; Нужен 👷🏽 на 🏗️ сегодня, завтра - оплата в конце дня... \n\n"
