...
async def show_write_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "Публикуем в канал 💁‍♂️Будапешт: Объявления, новости, подслушанно, жалобы.\n"
        "В канал 🕵🏽‍♀️Куплю/Отдам/Продам - все заявки связаны с торгами, обменами, раздачами и т.д.\n\n"
        
        "💥 *Заявка в 🙅🏻‍♀️Каталог Услуг*\n"
        "📰 Список специалистов, отсортированный по #️⃣ разным направлениям для удобного поиска пользователями❕\n"
        "(Примеры: делаю маникюр, репетитор английского, переводчик и т.д.)\n\n"
        
        "⚡️ *Актуальное*\n"
        "Важные и срочные сообщения, которые публикуются в наш чат и закрепляются❕\n"
        "(*Примеры*: нужен стоматолог на сегодня кто может принять❔; Потерял 🐶 в 13 районе➕ фото; "
        "Ищу на сегодня 🚐 для переезда по Будапешту; Нужен 👷🏽 на завтра - оплата в конце дня)\n\n"
        
        "🚶‍♀️ *Читать* - вернуться в главное меню"
    )
